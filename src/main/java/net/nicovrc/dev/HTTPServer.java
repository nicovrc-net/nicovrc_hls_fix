...
public class HTTPServer extends Thread {
    ...
    public byte[] hls_create(String httpVersion, String uri) throws Exception{
        ...
        System.out.println("ffmpeg -i https://yobi.nicovrc.net" + uri + " -c:v copy -c:a copy -f hls -hls_playlist_type vod -hls_segment_filename \"/hls/"+s+"/%3d.ts\" /hls/"+s+"/main.m3u8 > /dev/null");
        ProcessBuilder pb = new ProcessBuilder("/bin/bash", "-c", "ffmpeg -i https://yobi.nicovrc.net" + uri + " -c:v copy -c:a copy -f hls -hls_playlist_type vod -hls_segment_filename /hls/"+s+"/%3d.ts /hls/"+s+"/main.m3u8 > /dev/null");
